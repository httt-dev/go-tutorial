...
func getTableColumns(ctx context.Context, db *pgxpool.Pool, tableName string) ([]ColumnInfo, error) {
	query := `
	...
	rows, err := db.Query(ctx, query, strings.ToLower(tableName))
	if err != nil {
	...
	if rows.Err() != nil {
		return nil, fmt.Errorf("error iterating column info: %w", rows.Err())
	}
	if len(columns) == 0 {
...
func main() {
    ...
    srcPostgresDSN = os.Getenv("SRC_POSTGRES_DSN")
    ...
    if srcPostgresDSN == "" || dstOracleDSN == "" || expectedDBName == "" {
        log.Fatal("Please set the environment variables SRC_POSTGRES_DSN, DST_ORACLE_DSN, DESTINATION_DB_NAME")
    }
    ...
    srcPostgresDB, err := createPostgresPool(srcPostgresDSN)
	if err != nil {
		log.Fatalf("Error creating SOURCE PostgreSQL pool: %v", err)
	}
	defer srcPostgresDB.Close()
	...
	for _, mapping := range tableMappings {
        ...
        err = migrateTable(ctx, cancel, srcPostgresDB, dstOracleDB, srcTable, dstTable, partitionBy, partitionColumn, filterWhere)
        if err != nil {
...
func migrateTable(ctx context.Context, cancel context.CancelFunc, srcDB  *pgxpool.Pool, dstDB *sql.DB, srcTable string, dstTable string, partitionBy string, partitionColumn string, filterWhere string) error {
    ...
    hasClob,err := hasClobColumn(dstDB, srcTable)
    if err != nil {
    ...
    columnsInfo, err := getTableColumns(ctx, srcDB, srcTable)
    if err != nil {
    ...
    if strings.ToLower(partitionBy) == "rownum" {
        ...
        if err := srcDB.QueryRow(ctx, countQuery).Scan(&totalRows); err != nil {
        ...
        for i := 0; i < partitionCount; i++ {
            ...
            go func(startRow, endRow int64, partitionId int) {
                ...
                query := fmt.Sprintf(
                    "SELECT %s FROM (SELECT t.*, row_number() OVER (ORDER BY ctid) AS rnum FROM %s t WHERE 1=1 %s) sub WHERE rnum BETWEEN %d AND %d",
                    colList, srcTable, filterWhere, startRow, endRow,
                )
                ...
                rows, err := srcDB.Query(ctx, query)
                if err != nil {
                ...
                //     pgTypeOID := fieldDescs[i].DataTypeOID
                //     switch pgTypeOID {
                //     case 25, 1043, 1042, 17: // 25=text, 1043=varchar, 1042=char, 17=bytea
        ...
        row := srcDB.QueryRow(ctx, queryMinMax)
